...
```bash
git clone https: //github.com/mukundaparajuli/fixr.git
cd fixr
```
...
The backend uses environment variables prefixed with `FIXR_`. Key variables include:

- `FIXR_DATABASE_*` - PostgreSQL connection settings
- `FIXR_SERVER_*` - Server configuration
- `FIXR_AUTH_*` - Authentication settings
- `FIXR_REDIS_*` - Redis connection
- `FIXR_EMAIL_*` - Email service configuration
- `FIXR_OBSERVABILITY_*` - Monitoring settings

See `apps/backend/.env.example` for a complete list.

## Architecture

This fixr follows clean architecture principles:
